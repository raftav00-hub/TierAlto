...
function getBotResponse(input) {
  ...
  if (q.includes('number') || q.includes('contact') || q.includes('email') || q.includes('reach')) {
    return 'You can reach us at <a href="tel:16786995935" style="color:#028090">1-678-699-5935</a> or <a href="mailto:info@tieralto.com" style="color:#028090">info@tieralto.com</a>.';
  }
...
(function () {
  ...
  // Exit popup HTML not in page — abort silently
  if (!overlay) return;
  ...
  // Desktop only: fire when cursor moves toward top of browser window
  ...
  // Form submit — capture email for Field Notes newsletter
  form?.addEventListener('submit', function (e) {
    ...
    // Try to post to newsletter API — show success regardless
    fetch('/api/newsletter', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: email, source: 'Exit Intent Popup — Field Notes' }),
    }).catch(function () {
      // API not wired yet — fail silently, still show success to user
    });

    // Show success message in place of form
    if (form) {
      form.innerHTML =
        '<p id="ta-exit-success" style="color:#028090;font-weight:600;font-size:15px;padding:10px 0;">' +
